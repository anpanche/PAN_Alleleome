...
def analyze_gene_lengths(pangenome_alignments_dir_path, alleleome_dir_path):
    try:
        ...
        df2.to_csv(os.path.join(alleleome_dir_path, 'nuc_alleles_with_locus_mean_std.csv'))
        df2['mean_2std']=(df2['mean']- 2*df2['std'])
        df3=df2[df2.Length_of_allele < df2.mean_2std]
        df3.to_csv(os.path.join(alleleome_dir_path,'nuc_alleles_with_length_less_than_2std_less_than_mean_length.csv'))
